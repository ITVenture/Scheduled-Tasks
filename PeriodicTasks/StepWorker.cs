...
namespace PeriodicTasks
{
    ...
    public abstract class StepWorker:IConfigurablePlugin
    {
        ...
        public async Task<object> Run(PeriodicTask task, string command, Dictionary<StepParameter, object> values, IResourceLock @lock)
        {
            bool ok = RegisterRun();
            if (ok)
            {
                try
                {
                    if (SupportAsync)
                    {
                        return await RunTaskAsync(task, command, values, @lock);
                    }
                    else
                    {
                        return @lock.Exclusive(()=>RunTask(task, command, values));
                    }
                }
                finally
                {
        ...
        protected virtual Task<object> RunTaskAsync(PeriodicTask task, string command, Dictionary<StepParameter, object> values, IResourceLock @lock)
        {
            var retVal = @lock.Exclusive(() => RunTask(task, command, values));
