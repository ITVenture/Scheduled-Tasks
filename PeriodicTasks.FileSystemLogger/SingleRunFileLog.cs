...
{
    public class SingleRunFileLog : FileLog
    {
        /// <summary>
        /// Creates a new instance of the SingleRunFileLog class
        /// </summary>
        /// <param name="environment">the environment running tasks</param>
        /// <param name="logDirectory">the directory into which to log events</param>
        /// <param name="fileRetention">the number of days to keep log-files before deletion</param>
        public SingleRunFileLog(PeriodicEnvironment environment, string logDirectory, int fileRetention)
            : base(environment, logDirectory, fileRetention)
        {
        }
        ...
        {
            string logFileName = string.Format(@"{0}\{1}_{2:yyyyMMddHHmmss}.log", logDirectory, target.Name,
                                               DateTime.Now);
            var writer = new StreamWriter(logFileName, false, System.Text.Encoding.Default);
